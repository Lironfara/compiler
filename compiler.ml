 ...
 *)
 ...
 module Code_Generation (* : CODE_GENERATION *) = struct
   ...
   let collect_constants =
     ...
     | ScmApplic'(proc, arguments, app_kind) -> runs (proc::arguments)
     | ScmVarDef' (Var' (v, Free), expr') -> [ScmString v] @ (run expr')
     | ScmVarDef' (Var' (v, _), expr') -> (run expr')
     | ScmVarSet' (Var' (v, Free), expr') -> [ScmString v] @ (run expr')
     | ScmVarSet' (Var' (v, _), expr') -> (run expr')
 ...
 ScmLambda' (["a"; "b"], Simple,
  ScmApplic' (ScmVarGet' (Var' ("+", Free)),
   ...
   let collect_free_vars =
     let rec run = function
       | ScmVarDef' (Var'(name, Free), expr) -> [name]@(runs [expr])
...
(*
         ...
         ^"\tmov rbx, qword [rsp+8 * 1] \n"
         ^"\tmov qword[rbp+8*1], rbx\n"
       ...
       ^"\tcmp rdi, qword[rsp + 8*3]\t ;if rdi is equal to the number of parans - we donec\n"
       ^(Printf.sprintf"\tje %s\n" loop_end)
       ...
       ^(Printf.sprintf("\tlea rsp, [rbp + 8*1]\t; points to the ret adress of f\n")) (*rsp points to the top of the stack*)
       ^("\tpop rbp\t ;restore the rbp of f\n")
 *)
